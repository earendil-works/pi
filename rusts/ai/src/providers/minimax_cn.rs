...
fn models() -> Vec<Model> { vec![
    Model { id: "minimax-m1".into(), name: "MiniMax M1".into(), api: Api::AnthropicMessages, provider: ProviderId::Known(KnownProvider::MinimaxCN), base_url: "https://api.minimaxi.com/anthropic".into(), reasoning: true, thinking_level_map: HashMap::new(), input: vec![InputModality::Text], cost: ModelCost { input: 0.50, output: 2.0, cache_read: 0.0, cache_write: 0.0 }, context_window: 1_000_000, max_tokens: 32_768, headers: HashMap::new() },
]}

pub struct MinimaxCnProvider { auth: AuthMethod, models: Vec<Model> }
impl MinimaxCnProvider { pub fn new() -> Self { MinimaxCnProvider { auth: AuthMethod::ApiKey(ApiKeyAuth { name: "MiniMax CN API key".into(), env_vars: vec!["MINIMAX_CN_API_KEY".into()] }), models: models() } } }

#[async_trait]
impl Provider for MinimaxCnProvider {
    fn id(&self) -> &str { "minimax-cn" } fn name(&self) -> &str { "MiniMax CN" }
    fn base_url(&self) -> Option<&str> { Some("https://api.minimaxi.com/anthropic") } fn headers(&self) -> Option<&ProviderHeaders> { None }
...
#[cfg(test)] mod tests { use super::*; #[test] fn test_id() { assert_eq!(MinimaxCnProvider::new().id(), "minimax-cn"); } }

...
fn models() -> Vec<Model> { vec![
    Model { id: "accounts/fireworks/models/llama4-maverick-17b-128e-instruct".into(), name: "Llama 4 Maverick".into(), api: Api::OpenAiCompletions, provider: ProviderId::Known(KnownProvider::Fireworks), base_url: "https://api.fireworks.ai/inference/v1".into(), reasoning: false, thinking_level_map: HashMap::new(), input: vec![InputModality::Text], cost: ModelCost { input: 0.20, output: 0.60, cache_read: 0.0, cache_write: 0.0 }, context_window: 128_000, max_tokens: 8_192, headers: HashMap::new() },
    Model { id: "accounts/fireworks/models/deepseek-r1".into(), name: "DeepSeek R1".into(), api: Api::OpenAiCompletions, provider: ProviderId::Known(KnownProvider::Fireworks), base_url: "https://api.fireworks.ai/inference/v1".into(), reasoning: true, thinking_level_map: HashMap::new(), input: vec![InputModality::Text], cost: ModelCost { input: 3.0, output: 8.0, cache_read: 0.0, cache_write: 0.0 }, context_window: 128_000, max_tokens: 32_768, headers: HashMap::new() },
]}

pub struct FireworksProvider { auth: AuthMethod, models: Vec<Model> }
impl FireworksProvider { pub fn new() -> Self { FireworksProvider { auth: AuthMethod::ApiKey(ApiKeyAuth { name: "Fireworks API key".into(), env_vars: vec!["FIREWORKS_API_KEY".into()] }), models: models() } } }
#[async_trait]
impl Provider for FireworksProvider {
    fn id(&self) -> &str { "fireworks" } fn name(&self) -> &str { "Fireworks" } fn base_url(&self) -> Option<&str> { Some("https://api.fireworks.ai/inference/v1") } fn headers(&self) -> Option<&ProviderHeaders> { None } fn auth(&self) -> &AuthMethod { &self.auth } fn get_models(&self) -> &[Model] { &self.models }
...
#[cfg(test)] mod tests { use super::*; #[test] fn test_id() { assert_eq!(FireworksProvider::new().id(),"fireworks"); } }

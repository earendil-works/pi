...
describe("AgentSession concurrent prompt guard", () => {
	...
	it("sendUserMessage() should default to literal text but allow opting into prompt expansion", async () => {
		const model = getModel("anthropic", "claude-sonnet-4-5")!;
		const agent = new Agent({
			getApiKey: () => "test-key",
			initialState: {
		...
		(globalThis as typeof globalThis & { testCommandRuns?: number }).testCommandRuns = 0;
		const extensionsResult = await createTestExtensionsResult([
			(pi) => {
				pi.registerCommand("ping", {
					description: "test command",
					handler: async () => {
						(globalThis as typeof globalThis & { testCommandRuns?: number }).testCommandRuns =
							((globalThis as typeof globalThis & { testCommandRuns?: number }).testCommandRuns ?? 0) + 1;
					},
				});
			},
		]);
		...
		await session.sendUserMessage("/ping");
		expect((globalThis as typeof globalThis & { testCommandRuns?: number }).testCommandRuns).toBe(0);

		await session.sendUserMessage("/ping", { expandPromptTemplates: true });
		expect((globalThis as typeof globalThis & { testCommandRuns?: number }).testCommandRuns).toBe(1);
	});

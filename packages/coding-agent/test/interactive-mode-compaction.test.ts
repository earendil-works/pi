...
describe("InteractiveMode compaction events", () => {
	test("reconciles the persisted compaction summary exactly once", async () => {
		const fakeThis = {
			isInitialized: true,
			...
			reconcileCompletedTranscriptTurn: vi.fn(),
			showError: vi.fn(),
			showStatus: vi.fn(),
		...
		expect(fakeThis.reconcileCompletedTranscriptTurn).toHaveBeenCalledTimes(1);

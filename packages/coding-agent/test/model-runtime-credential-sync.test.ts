import type { ApiKeyCredential, Credential, CredentialStore, Model, Provider } from "@tculpepp/spi-ai";
...
describe("ModelRuntime credential synchronization", () => {
	...
	it("keeps provider-scoped refreshes from superseding unrelated providers", async () => {
		...
		// Drives the network-refresh branch against fake providers; the suite sets
		// SPI_OFFLINE, which would otherwise clamp allowNetwork to false.
		const runtime = await ModelRuntime.create({
			secureMode: false,
	...
	it("waits for a committed credential mutation to settle before reporting cancellation", async () => {
		...
		const runtime = await ModelRuntime.create({ secureMode: false, credentials, modelsPath: null });

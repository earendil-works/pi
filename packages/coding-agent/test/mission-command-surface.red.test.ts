...
interface RendererForMissionSlashTests {
	stop(): void;
...
async function makeRenderer(): Promise<{ renderer: RendererForMissionSlashTests; cleanup: () => void }> {
	initTheme("dark");
	const baseDir = mkdtempSync(join(tmpdir(), "mu-mission-command-surface-red-"));
	...
	return {
		renderer: renderer as unknown as RendererForMissionSlashTests,
		cleanup() {
			renderer.stop();
...
describe("/mission-run slash command (red)", () => {
	...
	it("registers a /mission-run command in the live TUI renderer", async () => {
		...
		expect(names).toContain("mission-run");
		expect(commands.find((command) => command.name === "mission-run")?.description).toMatch(/mission/i);
	});
});

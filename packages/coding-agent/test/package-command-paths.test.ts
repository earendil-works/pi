...
describe("package commands", () => {
	...
	let originalLylaPackageDir: string | undefined;
	...
	beforeEach(() => {
		...
		originalLylaPackageDir = process.env.LYLA_PACKAGE_DIR;
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("lyla install <source> [-l]");
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain('Use "lyla --help" or "lyla install <source> [-l]".');
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: lyla install <source> [-l]");

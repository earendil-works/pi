...
interface RendererForAlwaysOnSlashTests {
	stop(): void;
...
async function makeRenderer(): Promise<{ renderer: RendererForAlwaysOnSlashTests; cleanup: () => void }> {
	initTheme("dark");
	const baseDir = mkdtempSync(join(tmpdir(), "mu-always-on-slash-surface-red-"));
	...
	return {
		renderer: renderer as unknown as RendererForAlwaysOnSlashTests,
		cleanup() {
			renderer.stop();
...
describe("/always-on slash command surface (red)", () => {
	...
	it("registers /always-on in the live TUI renderer", async () => {
		...
		expect(commands.find((command) => command.name === "always-on")?.description).toMatch(/always-on/i);
	});
});

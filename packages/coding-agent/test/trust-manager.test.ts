...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".spi", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".spi", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".spi", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".spi"), { recursive: true });
			writeFileSync(join(cwd, ".spi", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".spi"), { recursive: true, force: true });

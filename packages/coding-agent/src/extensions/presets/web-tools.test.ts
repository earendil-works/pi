...
describe("createWebSearchTool", () => {
	it("spawns websearch query with CLI-parity flags", async () => {
		...
		expect(res.details.projection?.version).toBe(1);
		expect(res.details.projection?.call?.text).toContain("websearch query");
		expect(res.details.projection?.summary?.text).toContain("ok · exit=0");
	});
});
...
describe("createFetchTool", () => {
	it("spawns webfetch with CLI-parity flags and parses next= from stderr", async () => {
		...
		expect(res.details.projection?.version).toBe(1);
		expect(res.details.projection?.call?.text).toContain("webfetch https://example.com");
		expect(res.details.projection?.call?.text).toContain("--browser");
		expect(res.details.projection?.call?.text).toContain("--max-length 200");
		expect(res.details.projection?.call?.text).toContain("--start-index 10");
		expect(res.details.projection?.summary?.text).toContain("next=167");
	});
});

...
export interface Settings {
	...
	disabledProviders?: string[]; // Provider IDs excluded from discovery, authentication, and use
...
export class SettingsManager {
	...
	getDisabledProviders(): string[] {
		return [
			...new Set((this.settings.disabledProviders ?? []).map((providerId) => providerId.trim()).filter(Boolean)),
		];
	}

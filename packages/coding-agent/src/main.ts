...
import { type ImageContent, modelsAreEqual } from "@tculpepp/spi-ai";
...
export async function main(args: string[], options?: MainOptions) {
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.SPI_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: SPI_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}

...
Use `pi --ansteel "topic"` to run a non-interactive engineering review under mandatory three-role governance. Tech Lead publishes an architecture, Staff Engineer and QA Engineer independently challenge the same snapshot, and Tech Lead must resolve every recorded issue. Staff and QA then independently verify each revision, with at most two revision rounds before rejection. Only after both verify can Tech Lead write consensus for the final Staff and QA sign-off. Reports are saved under the project's `.pi/ansteel-reports/` directory by default. Each role stage has a fail-closed 120-second deadline by default; configure `stageTimeoutMs` in `.pi/ansteel.json` when a reviewed project needs a different bounded limit.

`.pi/ansteel.json` is required. It must name three distinct, authenticated Pi `provider/model` values and can restrict each role to built-in review tools. Missing, duplicate, or unavailable role models reject the review; there is no fallback to the current Pi model and no fourth architecture-review or sign-off role. Model diversity supplements, but never replaces, cited evidence. See [Evidence-First Reviews](docs/ansteel.md) for the workflow, issue ledger, verification semantics, configuration, and report format.

...
- **Footer** - Working directory, session name, total token/cache usage (`↑` input, `↓` output, `R` cache read, `W` cache write, `CH` latest cache hit rate; significant cache misses print a notice in the transcript, and `/session` shows the cumulative re-billed cost), cost, context usage, current model

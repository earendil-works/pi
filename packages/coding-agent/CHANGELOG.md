...
- Deferred interactive startup-only work such as changelog rendering, `fd`/`rg` readiness, provider-count footer metadata, and keybinding migration, removed TypeBox compiler startup cost from theme loading, moved `models.json` validation off AJV/TypeBox startup work, split syntax-highlighting helpers out of the base theme module, lazily exposed extension SDK exports through the extension loader alias, deferred resource/system-prompt materialization until first use, and delayed RPC extension binding until the first non-`get_state` command so startup does less unrelated work.

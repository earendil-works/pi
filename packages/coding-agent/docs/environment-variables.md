...
Provider API-key variables are documented separately in [Provider Authentication](provider-reference.md#use-an-api-key-from-the-environment).
...
Provider credentials such as `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, and cloud-provider configuration are listed in [Provider Authentication](provider-reference.md#use-an-api-key-from-the-environment).

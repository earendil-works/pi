...
**Source:** [`@tculpepp/pi-tui`](https://github.com/tculpepp/secure-pi-mono/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@tculpepp/pi-tui";
...
```typescript
import { Container, type Focusable, Input } from "@tculpepp/pi-tui";
...
Import from `@tculpepp/pi-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@tculpepp/pi-tui";
```
...
```typescript
import { matchesKey, Key } from "@tculpepp/pi-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@tculpepp/pi-tui";
...
```typescript
import { getMarkdownTheme } from "@tculpepp/pi-coding-agent";
import { Markdown } from "@tculpepp/pi-tui";
...
For letting users pick from a list of options. Use `SelectList` from `@tculpepp/pi-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@tculpepp/pi-coding-agent";
import { DynamicBorder } from "@tculpepp/pi-coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@tculpepp/pi-tui";
...
For toggling multiple settings. Use `SettingsList` from `@tculpepp/pi-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@tculpepp/pi-coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@tculpepp/pi-tui";
...
import { matchesKey, truncateToWidth } from "@tculpepp/pi-tui";

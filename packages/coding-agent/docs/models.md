...
| `promptCacheKeyMode` | Controls `prompt_cache_key` for `openai-completions` and `openai-responses`: `auto` preserves each adapter's existing behavior, `enabled` sends it whenever caching is active, and `disabled` omits it. Default: `auto`. |
...
For a compatible proxy that accepts `prompt_cache_key`, set `compat.promptCacheKeyMode` to `enabled`. This forwards a stable session key with the default short cache retention; it does not require `PI_CACHE_RETENTION=long` or send `prompt_cache_retention: "24h"`. Set it to `disabled` for a Responses-compatible proxy that rejects the field. Set `supportsLongCacheRetention` to `false` if the proxy does not accept long retention. `cacheRetention: "none"` still omits the cache key, and session-affinity headers remain separately configured.

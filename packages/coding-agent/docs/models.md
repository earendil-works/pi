...
| `supportsGoogleThoughtSignatures` | Google-style `extra_content.google.thought_signature` on function-call parts (Gemini 3 via OpenAI-compatible endpoints): pi captures it from streamed tool-call chunks and echoes it back on replayed tool calls, without which these models reject the request after a tool call. Set for Gemini 3.x models on OpenRouter and GitHub Copilot; enable manually on other gateways (e.g. Vertex AI's OpenAI-compatible API). Default: `false`. |

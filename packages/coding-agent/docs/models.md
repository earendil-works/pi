...
| `supportsMidConvoSystemMessages` | Whether the model accepts `role: "system"` messages inside `messages`. When enabled, Pi delivers system prompt and tool loadout changes as appended system messages instead of replacing the top-level system prompt. Default: `false`; built-in Opus 4.8+, Fable, and Mythos models enable it in generated metadata. Models without it receive system messages as tagged user turns, so only set it on a faithful Anthropic Messages transport. |

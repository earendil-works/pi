...
Amazon Bedrock Mantle exposes select models through an OpenAI-compatible Responses API. It reuses the same AWS credentials as Amazon Bedrock; requests are signed with AWS SigV4 using your credentials automatically (or pass a long-lived `AWS_BEARER_TOKEN_BEDROCK` bearer token).

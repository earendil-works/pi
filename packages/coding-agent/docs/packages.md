...
By default, `install` and `remove` write to user settings (`~/.spi/agent/settings.json`). Use `-l` to write to project settings (`.pi/settings.json`) instead. Project settings can be shared with your team, and pi installs any missing packages automatically on startup after the project is trusted.
...
- User installs go under `~/.spi/agent/npm/`.
...
- Cloned to `~/.spi/agent/git/<host>/<path>` (global) or `.pi/git/<host>/<path>` (project).
...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@tculpepp/spi-ai`, `@tculpepp/spi-agent-core`, `@tculpepp/spi-coding-agent`, `@tculpepp/spi-tui`, `typebox`.
...
Use `pi config` to enable or disable extensions, skills, prompt templates, and themes from installed packages and local directories. `pi config` starts in global settings (`~/.spi/agent/settings.json`); press Tab to switch between global and project-local modes. Use `pi config -l` to start in project overrides (`.pi/settings.json`) with inherited global resources dimmed.

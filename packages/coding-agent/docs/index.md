...
```bash
npm install -g --ignore-scripts @mupt-ai/pi-coding-agent
```
...
```bash
npm uninstall -g @mupt-ai/pi-coding-agent
```

For pnpm, Yarn, or Bun installs, use the matching global remove command: `pnpm remove -g @mupt-ai/pi-coding-agent`, `yarn global remove @mupt-ai/pi-coding-agent`, or `bun uninstall -g @mupt-ai/pi-coding-agent`.

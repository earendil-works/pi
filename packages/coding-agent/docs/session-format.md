...
A message in the conversation. The `message` field contains an `AgentMessage`. A new coding-agent session persists a first system message containing the complete initial prompt and tool loadout; provider requests derive both exclusively from that transcript state. Later system messages are persisted here as ordinary messages.
...
Optional fields:
- `systemMessage`: Complete prompt and tool state projected before the summary. It is absent on older session entries.

...
| Package | Purpose |
|---------|---------|
| `@lyla/coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@lyla/ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@lyla/tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@lyla/coding-agent";
...
```typescript
import { isBashToolResult } from "@lyla/coding-agent";
...
```typescript
import { createLocalBashOperations } from "@lyla/coding-agent";
...
```typescript
import { SessionManager } from "@lyla/coding-agent";
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@lyla/ai";
...
```typescript
import type { AutocompleteItem } from "@lyla/tui";
...
```typescript
import { withFileMutationQueue } from "@lyla/coding-agent";
...
**Important:** Use `StringEnum` from `@lyla/ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@lyla/coding-agent";
...
```typescript
import { createBashTool } from "@lyla/coding-agent";
...
```typescript
import { keyHint } from "@lyla/coding-agent";
...
```typescript
import { Text, Component } from "@lyla/tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@lyla/coding-agent";
import { matchesKey } from "@lyla/tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@lyla/coding-agent";

...
| Package | Purpose |
|---------|---------|
| `@mupt-ai/pi-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@mupt-ai/pi-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
...
```typescript
import { isToolCallEventType } from "@mupt-ai/pi-coding-agent";
...
```typescript
import { isBashToolResult } from "@mupt-ai/pi-coding-agent";
...
```typescript
import { createLocalBashOperations } from "@mupt-ai/pi-coding-agent";
...
```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@mupt-ai/pi-coding-agent";
...
```typescript
import { SessionManager } from "@mupt-ai/pi-coding-agent";
...
Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `@mupt-ai/pi-ai`. The provider becomes the composition base and `models.json` overrides still apply above it.

```typescript
import { createProvider, openAICompletionsApi } from "@mupt-ai/pi-ai";
...
```typescript
import { withFileMutationQueue } from "@mupt-ai/pi-coding-agent";
...
**Important:** Use `StringEnum` from `@mupt-ai/pi-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@mupt-ai/pi-coding-agent";
...
```typescript
import { createBashTool } from "@mupt-ai/pi-coding-agent";
...
```typescript
import { keyHint } from "@mupt-ai/pi-coding-agent";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@mupt-ai/pi-coding-agent";

...
> **Placement for /reload:** Put extensions in `~/.spi/agent/extensions/` (global) or `.spi/extensions/` (project-local) for auto-discovery. Use `spi -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.
...
| `.spi/extensions/*.ts` | Project-local |
| `.spi/extensions/*/index.ts` | Project-local (subdirectory) |
...
| Package | Purpose |
|---------|---------|
| `@tculpepp/pi-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `@sinclair/typebox` | Schema definitions for tool parameters |
| `@tculpepp/pi-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@tculpepp/pi-tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@tculpepp/pi-coding-agent";
...
```typescript
import { isBashToolResult } from "@tculpepp/pi-coding-agent";
...
```typescript
import { createLocalBashOperations } from "@tculpepp/pi-coding-agent";
...
```typescript
import { SessionManager } from "@tculpepp/pi-coding-agent";
...
```typescript
import type { AutocompleteItem } from "@tculpepp/pi-tui";
...
```typescript
import { withFileMutationQueue } from "@tculpepp/pi-coding-agent";
...
**Important:** Use `StringEnum` from `@tculpepp/pi-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@tculpepp/pi-coding-agent";
...
```typescript
import { createBashTool } from "@tculpepp/pi-coding-agent";
...
```typescript
import { keyHint } from "@tculpepp/pi-coding-agent";
...
```typescript
import { Text, Component } from "@tculpepp/pi-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@tculpepp/pi-coding-agent";
import { matchesKey } from "@tculpepp/pi-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@tculpepp/pi-coding-agent";

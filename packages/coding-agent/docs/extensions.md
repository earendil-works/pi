...
- `messageId` — the persisted session-entry id once the message has been written; `undefined` for live messages until persistence completes. Never fabricate a synthetic value for this field.
- `timestamp` — ISO timestamp from the session entry; same lifecycle as `messageId`
- `transientId` — stable per-render-session id generated by the presentation component so extensions can correlate streaming/width frames before `messageId` exists. This is not the session entry id.

Live user and assistant messages receive canonical `messageId`/`timestamp` via the existing `entry_appended` session event after the entry is appended (InteractiveMode attaches them to the live component). Restored/history rendering passes the same fields from the session entry directly.

Return the transformed Markdown:

```typescript
pi.registerMarkdownTransformer((markdown, { messageType, isStreaming, messageId, timestamp, transientId }) => {

...
Use `openrouter` for OpenRouter-style `reasoning: { effort }` controls. Use `together` for Together-style `reasoning: { enabled }` controls; with `supportsReasoningEffort`, it also sends `reasoning_effort`. Use `qwen-chat-template` instead for local Qwen-compatible servers that read `chat_template_kwargs.enable_thinking`. Use `chat-template` for vLLM/LiteLLM-hosted models with a configurable chat template kwarg. Set the kwarg name and value mode via `chatTemplateThinking` (default `{ key: "thinking", mode: "boolean" }`) and merge static kwargs via `chatTemplateKwargs` — e.g. DeepSeek-V3.x behind vLLM expects `chat_template_kwargs.thinking: true`.
...
interface ProviderModelConfig {
  ...
  compat?: {
    ...
    thinkingFormat?: "openai" | "openrouter" | "deepseek" | "together" | "zai" | "qwen" | "qwen-chat-template" | "chat-template";
    chatTemplateKwargs?: Record<string, unknown>;             // static kwargs merged into chat_template_kwargs (chat-template only)
    chatTemplateThinking?: { key?: string; mode?: "boolean" | "effort" };  // chat-template only; key defaults "thinking", mode defaults "boolean"
...
`openrouter` sends `reasoning: { effort }`. `deepseek` sends `thinking: { type: "enabled" | "disabled" }` and `reasoning_effort` when enabled. `together` sends `reasoning: { enabled }` and also `reasoning_effort` when `supportsReasoningEffort` is enabled. `qwen` is for DashScope-style top-level `enable_thinking`. Use `qwen-chat-template` for local Qwen-compatible servers that read `chat_template_kwargs.enable_thinking`. Use `chat-template` for vLLM-hosted models with a configurable kwarg name (`chatTemplateThinking.key`, default `"thinking"`) and value mode (`chatTemplateThinking.mode`, `"boolean"` or `"effort"`); merge static kwargs via `chatTemplateKwargs`.

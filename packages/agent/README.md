...
Stateful agent with tool execution and event streaming. Built on `@lyla/ai`.

## Installation

```bash
npm install @lyla/agent
```

## Quick Start

```typescript
import { Agent } from "@lyla/agent";
...
```typescript
import { Agent, streamProxy } from "@lyla/agent";
...
```typescript
import { agentLoop, agentLoopContinue } from "@lyla/agent";

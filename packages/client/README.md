...
```ts
import { PiClient, type ByteTransportFactory } from "@tculpepp/spi-client";
...
```ts
import { PiClient } from "@tculpepp/spi-client";
import { createUnixTransportFactory } from "@tculpepp/spi-client/unix";
...
The `@tculpepp/spi-client` root remains transport- and runtime-neutral. Importing the Node-compatible transport requires the explicit `@tculpepp/spi-client/unix` subpath.

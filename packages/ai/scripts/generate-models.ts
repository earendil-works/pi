...
async function generateModels() {
	...
	// Volcengine Ark Agent Plan / Coding Plan models
	// These are subscription-based plans with OpenAI-compatible endpoints.
	// Costs are zero because billing is plan-based (AFP / quota), not per-token.
	{
		const volcengineArkAgentModels: Model<"openai-completions">[] = [
			{
				id: "doubao-seed-evolving",
				name: "Doubao Seed Evolving",
				api: "openai-completions",
				baseUrl: "https://ark.cn-beijing.volces.com/api/plan/v3",
				provider: "volcengine-ark-agent",
				reasoning: true,
				input: ["text"],
		...
		allModels.push(...volcengineArkAgentModels);

		const volcengineArkCodingModels: Model<"openai-completions">[] = [
			{
		...
		allModels.push(...volcengineArkCodingModels);
	}
	...
	for (const candidate of allModels) {
		if (
			...
			candidate.provider !== "volcengine-ark-agent" &&
			candidate.provider !== "volcengine-ark-coding"
		) {

...
async function loadModelsDevData(): Promise<Model<any>[]> {
	try {
		...
		// Process CoralBricks models
		if (data.coralbricks?.models) {
			for (const [modelId, model] of Object.entries(data.coralbricks.models)) {
				const m = model as ModelsDevModel;
				if (m.tool_call !== true) continue;
				...
				recordModelsDevReasoningOptions("coralbricks", modelId, m);
			}
		}

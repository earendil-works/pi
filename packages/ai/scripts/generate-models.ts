...
// GMI Cloud is an OpenAI-Chat-Completions aggregator. Its reasoning models relay
// DeepSeek/GLM-style `reasoning_content`, so replayed assistant messages must carry the
// field. Thinking is still requested OpenAI-style (`reasoning_effort`) — verified live —
// so thinkingFormat stays at its "openai" default, the same pairing Fireworks uses for
// Kimi K3. Everything else GMI needs is what detectOpenAICompletionsCompat already
// defaults to, so no detection entry is added for api.gmi-serving.com.
// GMI multiplexes many upstreams, so its compat profile is the lowest common
// denominator across them. `role: "developer"` is rejected by the zai-org (GLM) and
// Qwen upstreams (HTTP 400 InvalidParameter) while `role: "system"` is accepted by
// every family tested, so developer-role is off provider-wide. Everything else GMI
// needs matches what detectOpenAICompletionsCompat already defaults to — both
// max_tokens and max_completion_tokens, store, reasoning_effort and strict-mode tools
// were verified live — so no detection entry is added for api.gmi-serving.com.
// Spread these into each model rather than sharing the object by reference: later
// generator passes mutate `model.compat` in place, and a shared literal would leak one
// model's mutation into every other model that still points at it.
const gmiCompat: OpenAICompletionsCompat = {
	supportsDeveloperRole: false,
	// Pinned, not left to detection: detectOpenAICompletionsCompat matches "deepseek-v4"
	// in a model id and would switch these to the "deepseek" branch, which sends a
	// top-level `thinking` object. GMI rejects that with
	// `Unsupported conversion: cursor_claude -> openai_chat_completion` and honours plain
	// `reasoning_effort` instead (verified live on deepseek-v4-flash-vision-exp).
	thinkingFormat: "openai",
};

// GMI's reasoning models relay DeepSeek/GLM-style `reasoning_content`, so replayed
// assistant messages must carry the field. Thinking is still requested OpenAI-style
// (`reasoning_effort`) — verified live — so thinkingFormat stays at its "openai"
// default, the same pairing Fireworks uses for Kimi K3.
const gmiReasoningCompat: OpenAICompletionsCompat = {
	...gmiCompat,
	requiresReasoningContentOnAssistantMessages: true,
};

// GMI Cloud publishes a catalog at GET /v1/models, but that endpoint requires an API key
// and this generator runs unauthenticated in CI — an empty fetch would delete the shard
// and break the build. So the catalog is snapshotted here instead, the same way ant-ling
// and deepseek are. models.dev also mirrors GMI (as "gmicloud") but lags it: 15 of 86
// models, 12 with stale prices, and context windows wrong in both directions.
//
// Snapshotted 2026-09-14 from GMI's own catalog, with `reasoning`, `input` and
// `maxTokens` established by probing the live API (GET /v1/models exposes none of them).
// 65 of the 80 entries had `input` confirmed by an actual image request. The 15 below
// could not be probed — the key used was rate limited on them — so their `input` is
// inferred from a probed sibling in the same family, or left text-only where there was
// no sibling to infer from. Re-probe these if image handling ever looks wrong:
//   inferred image:     the ten anthropic/claude-* models, google/gemma-4-31b-it,
//                       Qwen/Qwen3-Next-80B-A3B-Instruct, zai-org/GLM-5.3
//   inferred text-only: nvidia/nemotron-3-ultra-550b-a55b,
//                       nvidia/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-BF16
// Excluded: Qwen3-Coder-480B-A35B-Instruct-FP8 and GLM-4.7-FP8 (deprecated upstream),
// claude-fable-5 (no available source), Kimi-K2-Thinking (no matching target server),
// and gpt-5.4-pro (Responses API only — it 400s on /v1/chat/completions).
//
// Refresh with:
//   curl -sH "Authorization: Bearer $GMI_API_KEY" https://api.gmi-serving.com/v1/models
// `pricing.*` there are per-token USD decimal strings and pi's ModelCost is USD per
// MILLION tokens, so every value below is the GMI string multiplied by 1e6;
// `pricing.overrides[].min_prompt_tokens` maps onto ModelCostTier.inputTokensAbove.
const GMI_MODELS_DATA: Model<"openai-completions">[] = [
	{
	...
	},
	{
		...
		reasoning: true,
		input: ["text", "image"],
		cost: { input: 0.57, output: 2.29, cacheRead: 0.23, cacheWrite: 0 },
		contextWindow: 163840,
		maxTokens: 65536,
		compat: { ...gmiReasoningCompat },
	},
	{
		...
		reasoning: false,
		input: ["text", "image"],
		cost: { input: 0.29, output: 1.14, cacheRead: 0.11, cacheWrite: 0 },
		contextWindow: 163840,
		maxTokens: 65536,
		compat: { ...gmiCompat },
	},
	{
		...
		reasoning: true,
		input: ["text", "image"],
		cost: { input: 0.29, output: 0.43, cacheRead: 0.03, cacheWrite: 0 },
		contextWindow: 163840,
		maxTokens: 65536,
		compat: { ...gmiReasoningCompat },
	},
	...
	},
	{
		...
		reasoning: false,
		input: ["text", "image"],
		cost: { input: 0.25, output: 1.5, cacheRead: 0.025, cacheWrite: 0.083 },
		contextWindow: 1048576,
		maxTokens: 65536,
		compat: { ...gmiCompat },
	},
	...
	},
	{
		...
		reasoning: true,
		input: ["text", "image"],
		cost: { input: 1.5, output: 9, cacheRead: 0.15, cacheWrite: 0.083 },
		contextWindow: 1048576,
		maxTokens: 65536,
		compat: { ...gmiReasoningCompat },
	},
	...
	},
	{
		...
		reasoning: false,
		input: ["text", "image"],
		cost: { input: 0.14, output: 0.4, cacheRead: 0.1, cacheWrite: 0 },
	...
	},
	{
		...
		reasoning: false,
		input: ["text", "image"],
		cost: { input: 0.57, output: 2.29, cacheRead: 0.14, cacheWrite: 0 },
		contextWindow: 262144,
		maxTokens: 8192,
		compat: { ...gmiCompat },
	},
	...
	},
	{
		...
		reasoning: false,
		input: ["text"],
		cost: { input: 0.8, output: 2.6, cacheRead: 0.1, cacheWrite: 0 },
		contextWindow: 262144,
		maxTokens: 65536,
		compat: { ...gmiCompat },
	},
	{
		...
		reasoning: false,
		input: ["text"],
		cost: { input: 0.15, output: 1, cacheRead: 0.05, cacheWrite: 0 },
		contextWindow: 65536,
		maxTokens: 65536,
		compat: { ...gmiCompat },
	},
	{
		...
		reasoning: true,
		input: ["text", "image"],
		cost: { input: 2.5, output: 15, cacheRead: 0.25, cacheWrite: 0, tiers: [{ inputTokensAbove: 272000, input: 5, output: 22.5, cacheRead: 0.5, cacheWrite: 0 }] },
		contextWindow: 409600,
		maxTokens: 65536,
		compat: { ...gmiReasoningCompat },
	},
	...
	},
	{
		...
		reasoning: true,
		input: ["text", "image"],
		cost: { input: 1, output: 6, cacheRead: 0.1, cacheWrite: 1.25, tiers: [{ inputTokensAbove: 272000, input: 2, output: 9, cacheRead: 0.2, cacheWrite: 2.5 }] },
		contextWindow: 1050000,
		maxTokens: 65536,
		compat: { ...gmiReasoningCompat },
	},
	...
	},
	{
		...
		reasoning: true,
		input: ["text", "image"],
		cost: { input: 2.5, output: 15, cacheRead: 0.25, cacheWrite: 3.125, tiers: [{ inputTokensAbove: 272000, input: 5, output: 22.5, cacheRead: 0.5, cacheWrite: 6.25 }] },
		contextWindow: 1050000,
		maxTokens: 65536,
		compat: { ...gmiReasoningCompat },
	},
	...
	},
	{
		...
		reasoning: false,
		input: ["text", "image"],
		cost: { input: 0.35, output: 1.4, cacheRead: 0.07, cacheWrite: 0 },
		contextWindow: 262144,
		maxTokens: 65536,
		compat: { ...gmiCompat },
	},
	{
		...
		reasoning: true,
		input: ["text", "image"],
		cost: { input: 0.35, output: 4.2, cacheRead: 0, cacheWrite: 0 },
		contextWindow: 262141,
		maxTokens: 131072,
		compat: { ...gmiReasoningCompat },
	},
	{
	...
	},
	{
		...
		reasoning: true,
		input: ["text", "image"],
		cost: { input: 1.65, output: 4.951, cacheRead: 0.206, cacheWrite: 2.063 },
		contextWindow: 262144,
		maxTokens: 131072,
		compat: { ...gmiReasoningCompat },
	},
	...
	},
	{
		...
		reasoning: true,
		input: ["text", "image"],
		cost: { input: 0.18, output: 0.6, cacheRead: 0.06, cacheWrite: 0, tiers: [{ inputTokensAbove: 16384, input: 0.24, output: 0.96, cacheRead: 0.09, cacheWrite: 0 }, { inputTokensAbove: 32768, input: 0.3, output: 1.2, cacheRead: 0.12, cacheWrite: 0 }] },
		contextWindow: 262144,
		maxTokens: 65536,
		compat: { ...gmiReasoningCompat },
	},
	...
	},
	{
		...
		reasoning: true,
		input: ["text", "image"],
		cost: { input: 2, output: 6, cacheRead: 0.5, cacheWrite: 0, tiers: [{ inputTokensAbove: 200000, input: 4, output: 12, cacheRead: 1, cacheWrite: 0 }] },
...
async function generateModels() {
	...
	allModels.push(...GMI_MODELS_DATA);
	...
	for (const candidate of allModels) {
		if (
			...
			// GMI relays DeepSeek over its own OpenAI-compatible shim: it honours
			// `reasoning_effort` and rejects the "deepseek" branch's top-level `thinking`
			// object with `Unsupported conversion: cursor_claude -> openai_chat_completion`.
			candidate.provider !== "gmi"
		) {

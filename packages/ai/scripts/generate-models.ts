...
async function generateModels() {
	...
	// Merge Gateway: an OpenAI-compatible gateway that routes to many model
	// providers behind a single API key. Models are referenced by their
	// "provider/model" catalog slug. Curated set of currently-available,
	// tool-calling models; see https://docs.merge.dev/merge-gateway.
	const MERGE_GATEWAY_BASE_URL = "https://api-gateway.merge.dev/v1/openai";
	const mergeGatewayCompat: OpenAICompletionsCompat = {
		supportsReasoningEffort: false,
		supportsUsageInStreaming: true,
	};
	const mergeGatewayModels: Model<"openai-completions">[] = [
		{
	...
	allModels.push(...mergeGatewayModels);

...
/**
 * The image generation endpoint is a fixed catalog rather than a listing API,
 * so its models are described here instead of fetched. Every model takes
 * reference images through `subject_reference`, hence the `image` input.
 */
export function createMiniMaxImageModels(): Record<
	keyof typeof MINIMAX_IMAGE_BASE_URLS,
	ImagesModel<"minimax-images">[]
> {
	const createProviderModels = (provider: keyof typeof MINIMAX_IMAGE_BASE_URLS) =>
		MINIMAX_IMAGE_MODEL_IDS.map(
			(id): ImagesModel<"minimax-images"> => ({
				id,
				name: id,
				api: "minimax-images",
				provider,
	...
	return {
		minimax: createProviderModels("minimax"),
		"minimax-cn": createProviderModels("minimax-cn"),
	};
}
...
function generateImageModelsFile(modelsByProvider: Record<KnownImagesProvider, ImagesModel<ImagesApi>[]>): string {

...
describe("openai-completions tool_choice", () => {
	...
	it("maps unsupported groq qwen3 reasoning levels to none", async () => {
	...
	it("omits tool_stream when explicitly unsupported by z.ai model metadata", async () => {
		const baseModel = getModel("zai", "glm-4.7")!;
		const model = { ...baseModel, compat: { ...baseModel.compat, zaiToolStream: false } } as const;
		const tools: Tool[] = [
			{
				name: "ping",
				description: "Ping tool",

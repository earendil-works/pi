...
const MUSIC_GENERATION_MODEL_IDS = ["music-3.0", "music-2.6", "music-3.0-free", "music-2.6-free"] as const;
const MUSIC_COVER_MODEL_IDS = ["music-cover", "music-cover-free"] as const;
const MUSIC_OUTPUT_FORMATS = ["url", "hex"] as const;
const MUSIC_STREAM_OUTPUT_FORMATS = ["hex"] as const;
const MUSIC_AUDIO_FORMATS = ["mp3", "wav", "pcm"] as const;
const MUSIC_URL_TTL_HOURS = 24;
const MUSIC_COVER_CAPABILITIES = {
	inputOneOf: ["audio_url", "audio_base64"] as const,
	inputMinSeconds: 6,
	inputMaxSeconds: 360,
	inputMaxMb: 50,
};

type MusicModelId = (typeof MUSIC_GENERATION_MODEL_IDS)[number] | (typeof MUSIC_COVER_MODEL_IDS)[number];
...
function createMusicCatalog<TProvider extends KnownMusicProvider>(
	...
	const modelIds: readonly MusicModelId[] = [...MUSIC_GENERATION_MODEL_IDS, ...MUSIC_COVER_MODEL_IDS];
	return Object.fromEntries(
		modelIds.map((id) => [
			id,
			{
				...
				streamOutputFormats: MUSIC_STREAM_OUTPUT_FORMATS,
				audioFormats: MUSIC_AUDIO_FORMATS,
				urlTtlHours: MUSIC_URL_TTL_HOURS,
				regionalFields: region === "cn_zh" ? (["aigc_watermark"] as const) : [],
				cover: MUSIC_COVER_MODEL_IDS.includes(id as (typeof MUSIC_COVER_MODEL_IDS)[number])
					? MUSIC_COVER_CAPABILITIES
					: undefined,
			},
		]),

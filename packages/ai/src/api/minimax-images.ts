...
export const generateImages: ImagesFunction<"minimax-images", MiniMaxImagesOptions> = async (
	...
	try {
		...
		for (const data of payload.data?.image_base64 ?? []) {
			const image = parseBase64Image(data);
			if (image) output.output.push(image);
		}
		for (const url of payload.data?.image_urls ?? []) {
			if (url) output.output.push(await resolveImageUrl(url, options));
		}
...
function buildParams(
	...
	const subjectReferences =
		options?.subjectReference !== undefined
			? options.subjectReference.map((reference) => ({
					type: reference.type,
			...
			: context.input
					...
					.map((image) => ({
						...
						image_file: `data:${image.mimeType};base64,${image.data}`,
					}));

	return {
		model: model.id,
		prompt,
		...(subjectReferences.length > 0 ? { subject_reference: subjectReferences } : {}),

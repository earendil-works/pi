...
function classifyCooldown(error) {
    ...
    const rateLimitHint = message.includes("rate limit") ||
...
export function getRetryDelay(attempt, baseDelay, maxDelay, error) {
    ...
    if (retryAfterMs === undefined)
        return Math.min(maxDelay, backoff);
    ...
    (async () => {
        ...
        while (true) {
            ...
            try {
                ...
                const canRetryWithoutContent = error instanceof CodexStreamError &&
                    !error.hadContent &&
                    isRetryableCodexError(error, options?.signal, retryOn);
                const canRetryStream = !options?.signal?.aborted &&
                ...
                const shouldRetryRequest = (!hasEmittedStart || canRetryWithoutContent) &&
                    isRetryableCodexError(error, options?.signal, retryOn) &&
...
async function processStream(response, output, stream, model, tools) {
        ...
        switch (type) {
            ...
            case "response.done": {
                ...
                if (output.stopReason === "stop" && !output.content.some((b) => b.type === "toolCall")) {
                    const recoveredToolCall = recoverToolCallFromTextContent(output.content, tools, (name) => normalizeToolName(name, tools), (raw) => parseToolArgumentsSafely(raw));

...
function detectCompatFromUrl(baseUrl) {
    ...
    const isNonStandard = isMoonshot ||
    ...
    const useMaxTokens = isMoonshot || baseUrl.includes("mistral.ai") || baseUrl.includes("chutes.ai") || baseUrl.includes("fireworks.ai");
...
function splitThinkSegments(input, state) {
    const segments = [];
    state.buffer += input;
...
function flushThinkParseState(state) {
    if (state.buffer.length === 0)
        return [];
    const segment = {
...
function parsePossiblyDoubleEncoded(jsonStr) {
    const first = parseStreamingJson(jsonStr);
...
function getNonEmptyStringField(source, field) {
    ...
    (async () => {
        ...
        try {
            ...
            for await (const chunk of openaiStream) {
                ...
                if (!choice)
                    continue;
                const choiceWithMessage = choice;
                if (choice.finish_reason) {
                    output.stopReason = mapStopReason(choice.finish_reason);
                }
                const contentDelta = getNonEmptyStringField(choice.delta, "content") ??
                ...
                for (const field of reasoningFields) {
                    const reasoningDelta = getNonEmptyStringField(choice.delta, field) ??
                ...
                const toolCalls = choice.delta?.tool_calls && choice.delta.tool_calls.length > 0
                ...
                if (toolCalls) {
                    for (const toolCall of toolCalls) {
                        ...
                        const hasName = toolCall.function?.name !== null &&
...
function buildParams(model, context, options) {
    ...
    if (options?.reasoningEffort && model.reasoning) {
        ...
        if (isBasetenThinkingModel(model)) {
            ...
            const chatTemplateArgs = existingChatTemplateArgs &&
        ...
        const allowReasoningEffort = compat.supportsReasoningEffort && (!isBasetenBaseUrl(model.baseUrl) || isBasetenReasoningEffortModel(model));
            ...
            if (thinkingBlocks.length > 0) {
                ...
                if (joinedThinking.length > 0) {
                    if (model.reasoningFormat === "reasoning_content") {
                        ...
                        assistantMsg.reasoning_content = joinedThinking;
                    }

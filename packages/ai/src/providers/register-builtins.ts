...
const importNodeOnlyProvider = (specifier: string): Promise<unknown> => {
	const runtimeSpecifier = import.meta.url.endsWith(".ts") ? specifier.replace(/\.ts$/, ".ts") : specifier;
...
> {
	googleGeminiCliProviderModulePromise ||= import("./google-gemini-cli.ts").then((module) => {

...
import { normalizeToolNameWithTools, recoverToolCallFromTextContent, upsertToolCallContent, } from "./tool-call-recovery.js";
...
function getErrorCode(error) {
    if (error instanceof OpenAIResponsesStreamError)
        return error.code;
    const record = asRecord(error);
    ...
    (async () => {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            ...
            try {
                    ...
                    // Handle completion
                        ...
                        if (response?.usage) {
                            ...
                            const rawOutput = usage.output_tokens ??
                                ...
                                0;
                            const cachedTokens = response.usage.input_tokens_details
                                ?.cached_tokens ?? 0;
                        ...
                        if (response?.status === "queued" || response?.status === "in_progress") {
                            throw new OpenAIResponsesStreamError(`Stream ended with non-terminal status: ${response.status}`, hadContent);
                        }
                        // Map status to stop reason
                        output.stopReason = mapStopReason(response?.status);
                        if (output.stopReason === "stop" && !output.content.some((b) => b.type === "toolCall")) {
                            const recoveredToolCall = recoverToolCallFromTextContent(output.content, context.tools, (name) => normalizeToolName(name, context.tools), (raw) => parseToolArgumentsSafely(raw));
                ...
                if (!sawCompletion) {
                    throw new OpenAIResponsesStreamError(hadContent ? "Stream terminated before completion" : "Stream terminated", hadContent);
                }
                ...
                lastError = error;
                const canRetryWithoutContent = error instanceof OpenAIResponsesStreamError &&
                    !error.hadContent &&
                    isRetryableOpenAIError(error, options?.signal);
                const shouldRetry = (!hasEmittedStart || canRetryWithoutContent) &&
                    isRetryableOpenAIError(error, options?.signal) &&
                    attempt < maxRetries;
                if (shouldRetry) {

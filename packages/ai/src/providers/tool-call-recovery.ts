...
function baseNormalizeToolName(name: string): string {
...
export function normalizeToolNameWithTools(name: string, tools: Tool[] | undefined): string {
	const raw = name.trim();
	if (!raw) return raw;

	const normalized = baseNormalizeToolName(raw);
	if (!tools || tools.length === 0) return normalized;
...
function buildRecoveredToolCallId(toolName: string, seed: string): string {
...
export function upsertToolCallContent(
	...
	toolCall: ToolCall,
): { contentIndex: number; inserted: boolean } {
...
export function recoverToolCallFromTextContent(
	content: AssistantMessage["content"],
	tools: Tool[] | undefined,
	normalizeToolName: (name: string) => string,
	parseArguments: (raw: string | undefined, toolName: string) => Record<string, unknown>,
): ToolCall | null {
	...
	for (let index = content.length - 1; index >= 0; index--) {
		...
		if (text.includes("```") || text.includes("`to=")) continue;
		...
		if (Object.keys(argumentsObject).length === 0) continue;

...
```bash
npm install @tculpepp/spi-ai
```

TypeBox exports are re-exported from `@tculpepp/spi-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@tculpepp/spi-ai';
import { builtinModels } from '@tculpepp/spi-ai/providers/all';
...
import { openaiProvider } from '@tculpepp/spi-ai/providers/openai';
import { openrouterProvider } from '@tculpepp/spi-ai/providers/openrouter';
import { amazonBedrockProvider } from '@tculpepp/spi-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@tculpepp/spi-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@tculpepp/spi-ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@tculpepp/spi-ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@tculpepp/spi-ai';
...
```typescript
import { validateToolCall, type Tool } from '@tculpepp/spi-ai';
...
```typescript
import { builtinImagesModels } from '@tculpepp/spi-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@tculpepp/spi-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@tculpepp/spi-ai';
import { openAICompletionsApi } from '@tculpepp/spi-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@tculpepp/spi-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@tculpepp/spi-ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@tculpepp/spi-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@tculpepp/spi-ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@tculpepp/spi-ai';
import { anthropicProvider } from '@tculpepp/spi-ai/providers/anthropic';
import { openaiProvider } from '@tculpepp/spi-ai/providers/openai';
import { googleProvider } from '@tculpepp/spi-ai/providers/google';
...
- `@tculpepp/spi-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@tculpepp/spi-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@tculpepp/spi-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@tculpepp/spi-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@tculpepp/spi-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@tculpepp/spi-ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@tculpepp/spi-ai/bedrock-provider';
...
Pass `env` in stream options to scope provider configuration to a request. Values in `env` are used before process environment variables for provider auth and configuration such as Cloudflare account IDs, Azure OpenAI settings, Vertex project/location, Bedrock settings, `SPI_CACHE_RETENTION`, and `HTTP_PROXY`/`HTTPS_PROXY`.
...
```bash
npx @tculpepp/spi-ai login              # interactive provider selection
npx @tculpepp/spi-ai login anthropic    # login to specific provider
npx @tculpepp/spi-ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@tculpepp/spi-ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@tculpepp/spi-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@tculpepp/spi-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@tculpepp/spi-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@tculpepp/spi-ai`.

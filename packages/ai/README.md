...
| Amazon Bedrock Mantle | AWS credentials via the standard chain (`AWS_PROFILE`, `AWS_ACCESS_KEY_ID` + `AWS_SECRET_ACCESS_KEY`, ECS/IRSA roles) or a long-lived `AWS_BEARER_TOKEN_BEDROCK`. Requests are signed with AWS SigV4 from the credential chain automatically. Built-in models use `AWS_REGION`, then `AWS_DEFAULT_REGION`, when the selected model is available there; otherwise they fall back to a supported region. |

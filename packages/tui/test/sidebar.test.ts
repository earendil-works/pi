...
describe("SidebarContainer", () => {
    it("empty config renders nothing", () => {
        let requested = false;
        const container = new SidebarContainer(() => { requested = true; });
    ...
    it("single tab renders content directly (no tab bar)", () => {
        ...
        container.updateConfig([
            { id: "a", label: "Panel A", component: new Text("Hello", 0, 0) },
        ]);
        const lines = container.render(80);
    ...
    it("updateConfig replaces all tabs", () => {
        const container = new SidebarContainer(() => {});
        container.updateConfig([
            { id: "a", label: "A", component: new Text("Old", 0, 0) },
        ]);
        container.updateConfig([
            { id: "b", label: "B", component: new Text("New", 0, 0) },
        ]);
    ...
    it("requestRender callback is triggered on switch", () => {
        let called = false;
        const container = new SidebarContainer(() => { called = true; });
        container.updateConfig([

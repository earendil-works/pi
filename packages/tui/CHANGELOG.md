...
- Hardened the `V2TUIHost` pager lifecycle so a throw after rendering is suspended (e.g. a resize whose reflow re-invokes a history source that throws) fails safe: the partial viewer is torn down, the primary screen is restored, coherent live rendering resumes with a single repaint, and the originating error is rethrown rather than swallowed (plan §6).

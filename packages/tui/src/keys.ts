...
export function matchesKey(data: string, keyId: KeyId): boolean {
	...
	if (key.length === 1 && ((key >= "a" && key <= "z") || isDigitKey(key) || SYMBOL_KEYS.has(key))) {
		...
		if (modifier === MODIFIERS.alt && (isLetter || isDigit)) {
			// Match alt+letter/digit regardless of Kitty protocol negotiation state.
			// Terminals that speak Kitty keyboard protocol (e.g. Ghostty, Kitty) may
			// send CSI-u sequences before Pi finishes protocol negotiation. Without
			// checking matchedKittySequence here, the first keystrokes after startup
			// (or before the first Kitty response arrives) are silently dropped.
			if (data === `\x1b${key}`) return true;
			return matchesKittySequence(data, codepoint, MODIFIERS.alt);
		}

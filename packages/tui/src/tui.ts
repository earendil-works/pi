...
export class TUI extends Container {
	...
	private doRender(): void {
		...
		const debugRedraw = process.env.LYLA_DEBUG_REDRAW === "1";
		const logRedraw = (reason: string): void => {
			if (!debugRedraw) return;
			const logPath = path.join(os.homedir(), ".lyla", "agent", "lyla-debug.log");
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				...
				const crashLogPath = path.join(os.homedir(), ".lyla", "agent", "lyla-crash.log");
				const crashData = [

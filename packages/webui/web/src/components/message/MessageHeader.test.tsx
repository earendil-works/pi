...
describe("MessageHeader", () => {
  ...
  it("truncates model name longer than 24 chars with an ellipsis", () => {
    const longModelName = "very-long-model-name-that-exceeds-24-chars-yes";
    render(<MessageHeader {...baseProps} model={longModelName} />);

    // 24 chars + ellipsis = first 24 + "…"
    const badge = screen.getByText("very-long-model-name-tha…");
    expect(badge).toBeTruthy();
  });
  ...
  it("does not truncate a 16-char model name (off-by-one bug regression test)", () => {
    ...
    // instead of "deepseek-v4-flash"). New impl uses maxLen=24 with
  ...
  it("prefixes model name with provider when provider is supplied", () => {
    render(
      <MessageHeader
        {...baseProps}
    ...
    const badge = screen.getByText("opencode-go/deepseek-v4-flash");
    expect(badge).toBeTruthy();
  });
});

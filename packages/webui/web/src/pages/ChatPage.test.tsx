...
describe("ChatPage", () => {
  ...
  describe("Ask user question placeholder", () => {
    it("receives extension_ui_request event → renders AskUserQuestionPending placeholder", async () => {
      await renderChatPage("s1");
      await waitFor(() => {
        expect(screen.queryByText("Loading...")).toBeNull();
      });
      ...
      await waitFor(() => {
        expect(screen.queryByTestId("ask-user-question-pending")).toBeInTheDocument();
      }, { timeout: 2000 });
    });

    it("tool_execution_end with toolName=ask_user_question removes the placeholder", async () => {
      await renderChatPage("s1");
      await waitFor(() => {
        expect(screen.queryByText("Loading...")).toBeNull();
      });
      ...
      // First: extension_ui_request appears
      handler!({
        type: "session_event",
        sessionId: "s1",
        event: {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("ask-user-question-pending")).toBeInTheDocument();
      });

      // Second: tool_execution_end for ask_user_question removes placeholder
      handler!({
        type: "session_event",
        sessionId: "s1",
        event: {
      ...
      await waitFor(() => {
        expect(screen.queryByTestId("ask-user-question-pending")).not.toBeInTheDocument();
      }, { timeout: 2000 });
    });

    it("tool_execution_end with non-matching toolName keeps the placeholder", async () => {
      await renderChatPage("s1");
      await waitFor(() => {
        expect(screen.queryByText("Loading...")).toBeNull();
      });
      ...
      // Second: tool_execution_end with toolName=bash (not ask_user_question) → placeholder stays
      handler!({
        type: "session_event",
        sessionId: "s1",
        event: {

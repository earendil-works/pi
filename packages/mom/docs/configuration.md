...
All variables are optional unless noted. **Slack thread/conversation UX** (reply in thread, tracked threads without @mention, hourglass/checkmark reactions, “On it / Done” status in thread) defaults to **on** when unset. Set **`MOM_SLACK_LEGACY_THREAD_UX=1`** to restore upstream-style defaults for those four flags together, or set each flag to `0` individually. Anthropic + `claude-sonnet-4-6` remains the default unless you override LLM vars.
...
| `MOM_SLACK_LEGACY_THREAD_UX` | off | If `1`: same as upstream when unset — `MOM_SLACK_REPLY_IN_USER_THREAD`, `MOM_TRACK_THREADS`, `MOM_SLACK_STATUS_REACTIONS`, and `MOM_SLACK_STATUS_THREAD_MESSAGE` default to off (unless overridden per var). |
| `MOM_SLACK_REPLY_IN_USER_THREAD` | **on** | If `1`: first bot message and streaming updates go under the user message thread instead of the channel root. |
| `MOM_TRACK_THREADS` | **on** | Persist `tracked-threads.json` in the working dir; allow follow-up messages in those threads **without** @mention. |
| `MOM_SLACK_STATUS_REACTIONS` | **on** | Hourglass on user message while running; then checkmark or x. **Needs `reactions:write`** on the bot; without it Slack returns `missing_scope` and only the text/thread status appears. |
| `MOM_SLACK_STATUS_THREAD_MESSAGE` | **on** | Post “On it!” / “Done” (or stopped) in the user thread. |

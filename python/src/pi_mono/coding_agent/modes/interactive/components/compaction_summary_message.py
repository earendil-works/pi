...
class CompactionSummaryMessageComponent(Box):
    ...
    def _update_display(self) -> None:
        ...
        tokens_before = int(self._message.get("tokensBefore") or 0)
        if self._expanded:
            header = f"**Compacted from {tokens_before:,} tokens**\n\n"
            self.add_child(
                Markdown(
            ...
            self.add_child(
                Text(
                    theme.fg("customMessageText", f"Compacted from {tokens_before:,} tokens (")

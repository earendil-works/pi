#!/usr/bin/env bash
#
# Build spi binaries for all platforms locally.
...
#     spi-darwin-arm64.tar.gz
#     spi-darwin-x64.tar.gz
#     spi-linux-x64.tar.gz
#     spi-linux-arm64.tar.gz
#     spi-windows-x64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    if [[ "$platform" == "windows-x64" ]]; then
        bun build --compile --external koffi --target=bun-$platform ./dist/bun/cli.js --outfile binaries/$platform/spi.exe
    else
        bun build --compile --external koffi --target=bun-$platform ./dist/bun/cli.js --outfile binaries/$platform/spi
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == "windows-x64" ]]; then
        # Windows (zip)
        echo "Creating spi-$platform.zip..."
        (cd $platform && zip -r ../spi-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating spi-$platform.tar.gz..."
        mv $platform spi && tar -czf spi-$platform.tar.gz spi && mv spi $platform
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf $platform
    if [[ "$platform" == "windows-x64" ]]; then
        mkdir -p $platform && (cd $platform && unzip -q ../spi-$platform.zip)
    else
        tar -xzf spi-$platform.tar.gz && mv spi $platform
    fi
done

...
const internalPackagePrefixes = ["@mupt-ai/pi-", "@earendil-works/pi-"];
const installPackageName = "@mupt-ai/pi-coding-agent-install";
...
function validateGeneratedFiles(installerPackageJson, installLock, internalNames) {
	...
	for (const [lockPath, entry] of Object.entries(installLock.packages)) {
		...
		const isVersionedMuptPackage = packageName?.startsWith("@mupt-ai/pi-") ?? false;
		if (isVersionedMuptPackage && entry.version !== installerPackageJson.version) {

...
- **`recallAtoms` RRF threshold aligned to strict design intent (`1/60`) + `rrfK`/`recallThreshold` config knobs exposed**: previously `DEFAULT_RECALL_THRESHOLD = 1 / (DEFAULT_RRF_K + 1)` let a single-channel rank=1 contribution (0.01639) clear the gate, defeating the "宁可漏召不可误召" principle and letting the user's `lefse` dense-noise case (cosine 0.55, BM25 0 hit) leak through. The threshold is now `1 / DEFAULT_RRF_K` (0.01667) — strict by default, so single-channel rank=1 contributions (0.01639) are correctly filtered; double-channel hits and dense+BM25 at any reasonable rank combinations still pass. The `RecallOptions` interface now exposes `rrfK` and `recallThreshold` for user override; `recallThreshold: 0` is the documented escape hatch for test/dev mode (single-channel BM25-only / dense-only rescue). Hybrid scenario tests (BM25-only, dense-only, double-channel, embedText null → pure BM25) now pass `{ recallThreshold: 0 }` to exercise the channel logic in isolation; `recall-quality.test.ts` does the same to measure pure ranking without the gate interfering. The lefse case (dense cosine 0.55, BM25 0 hit, rrfScore 0.01639 < 0.01667) is now correctly filtered.

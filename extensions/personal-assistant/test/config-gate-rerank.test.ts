...
// Type-level test for design.md D6 / spec R5 — PersonalAssistantConfig.memory
// must accept `gate: { enabled?: boolean }` and `rerank: { enabled?: boolean }`.
//
...
describe("PersonalAssistantConfig gate/rerank fields (D6 / spec R5)", () => {
	...
	it("backward compat: old settings.json shape (no gate / no rerank) still typechecks", () => {

...
## Capability: recall-multiquery

独立 query rewrite 阶段 + 多路 recall 合并,在 gate 二分与 cross-encoder rerank 之间扩展 pipeline,支持复合 query 多概念 fan-out.

### Requirements

#### Requirement: Query Rewrite Stage (rewrite.ts)
The system SHALL provide an independent `rewriteQueries` module that, given a user query and an optional list of recent user messages, returns 1-3 atomic subqueries via ollama qwen2.5:3b-instruct-q4_0 (temperature=0, timeout 5000ms). Function signature: `rewriteQueries(query: string, recent?: string[] | null, options?: RewriteOptions): Promise<string[] | RewriteFallback>`. Failed calls (timeout / parse / unreachable) return `RewriteFallback { reason, subqueries: [rawQuery] }` (single fallback query), enforced via `Array.isArray` discrimination at caller. The `maxSubqueries` option (default 3) caps the returned array.
...
##### Scenario: rewrite 超时 5000ms 降级 (E1)
- **GIVEN** ollama 5000ms 内未响应
- **WHEN** AbortController 触发
...
- **GIVEN** LLM 返 5 条 `["a","b","c","d","e"]`, maxSubqueries=3
- **WHEN** rewrite 处理 string[]
- **THEN** `slice(0, maxSubqueries)` 截到 `["a","b","c"]`,不抛错不警告,debug log 输出 `truncated 5→3`
...
The system SHALL provide two pure functions for merging per-subquery recall result groups:
- `mergeByAtomId(resultGroups: RecallResult[][]): RecallResult[]` — dedup by `atom.id`, keep the group with the highest `rrf` for each id; preserves the entire `cosine` + `sparseScore` + `rrf` triple from that group.
- `mergeByRerankScore(resultGroups: RecallResult[][]): RecallResult[]` — dedup by `atom.id`, keep the entry with the highest `rerankScore`; result is sorted by `rerankScore` DESC (default `-1` for missing so unscored entries cannot outrank scored ones).

`mergeByAtomId` is the rrf-based merge for non-rerank callers. `mergeByRerankScore` is the production per-subquery pipeline merge — the gate→rewrite→per-subquery recall+rerank→merge path uses `mergeByRerankScore` because rerank is the authoritative sort signal.

##### Scenario: 多组重叠 atomId 取 rrf 最高 (B4, mergeByAtomId)
...
##### Scenario: 多组重叠 atomId 取 rerankScore 最高 (mergeByRerankScore)
- **GIVEN** resultGroups = `[[a(rs=0.97)], [a(rs=0.34), b(rs=0.53)]]`
- **WHEN** mergeByRerankScore 执行
- **THEN** 输出 `[a(rs=0.97), b(rs=0.53)]` 降序
- **AND** a 来自第一组(分高),b 来自第二组
...
`POST /api/memory/search` 当 body `filtered=true` (default) 时 SHALL 走 rewrite + multi-recall + merge + rerank 全链路. 当 `filtered=false` 时仅 recallAtoms 单路返原 RRF 候选, 不调 rewrite / rerank. 输入验证 SHALL clamp `topK` to [1,100] (default 10) and require `type` ∈ {rule, fact, process} if provided. 端点 SHALL 受 60/min/IP rate limiter 保护 (loopback-only 但保护本地 misbehavior).
...
- **AND** response 不含 `rerankScore` / `rewriteTimeMs` / `rerankTimeMs` 字段

##### Scenario: topK 超上限 clamp (SEC)
- **GIVEN** POST body=`{query:"x", topK:9999}`
- **WHEN** server 处理
- **THEN** 内部 topK 被 clamp 到 100,recallAtoms 不会拿到 9999

##### Scenario: type 不在 allowlist 被忽略 (SEC)
- **GIVEN** POST body=`{query:"x", type:"../../etc"}`
- **WHEN** server 处理
- **THEN** type 被设为 undefined,行为等价于省略 type 字段
...
## Capability: agent-memory-write-tool

The `memory_save` tool is the agent-facing single entry point for durably writing facts, rules, and processes to memory. It replaces direct `.md` file writes via the `write` / `edit` / `bash` tools — the `tool_call` hook blocks those bypass paths.
...
## Capability: tui-webui-recall-parity

Both the TUI `context` hook and the webui `POST /api/memory/search` route share a single `recallPipeline()` helper. This eliminates drift between the two surfaces — topK, recent, rewrite, rerank, merge, status metadata are all driven by one function.

### Requirement: shared `recallPipeline` is the single recall entry point

Both the TUI `context` hook (personal-assistant `memory.ts`) and the webui `POST /api/memory/search` route (webui `routes/memory.ts`) SHALL invoke the same `recallPipeline(index, opts)` function from `extensions/personal-assistant/recall.ts`. No inline recall/rewrite/rerank/merge pipeline code SHALL remain in either caller after this change.
...
- **THEN** the handler returns HTTP 400 `{error: "recent must be a string[] when supplied"}`

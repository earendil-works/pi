...
- [x] 1.1 **`RecallResult` swaps `file_path` → `score`**
...
- [x] 1.2 **types test updates for new shape**
...
- [x] 3.1 **`formatMemoryBlock` emits `id:` line instead of `file:`**
...
- [x] 4.1 **Add `scoreUserTone` + integrate into `buildExtractionPrompt`**
...
- [x] 4.4 **index exports test update**
...
- [x] 5.1 **Define `MemoryGetParams` + register `memory_get` tool**
...
- [x] 6.1 **search route returns `score`, no `file_path`**
...
- [x] 6.2 **GET /:id preview-only — does not bump access_count**
  - **文件**: `packages/webui/server/routes/memory.ts` (Modify)
...
- [x] 6.3 **webui search test updates**
...
- [x] 6.4 **GET /:id no-bump regression test**
  - **文件**: `packages/webui/server/test/memory-routes.test.ts` (Modify)
...
- [x] 7.2 **MemorySearchTester uses `score` not `file_path`**
...
- [x] 7.3 **MemorySearchTester test rewrite**
...
- [x] 8.1 **personal-assistant CHANGELOG entry**
...
## Verification
- [x] personal-assistant tests: `cd /home/qjh/workspace/personal/pi/extensions/personal-assistant && ../../node_modules/.bin/vitest --run`
- [x] webui server tests: `cd /home/qjh/workspace/personal/pi/packages/webui/server && ../../../../node_modules/.bin/vitest --run`
- [x] webui frontend tests: `cd /home/qjh/workspace/personal/pi/packages/webui/web && ./node_modules/.bin/vitest --run --no-file-parallelism`
- [x] Full repo check: `cd /home/qjh/workspace/personal/pi && npm run check` (must exit 0; full output, no tail)
...
The webui MessageParts component SHALL wrap the assistant turn's **inference** content in a collapsible step container when the `parts: Part[]` array contains any of: `thinking`, `toolCall`, `toolResult`, or `image`. The fold's body contains, in chronological order: every `thinking` part, every `toolCall` / `toolResult` / `image` part (grouped into a ToolGroup when consecutive), AND every `text` part EXCEPT the LAST one. The LAST `text` part in the turn is the agent's final conclusion and renders OUTSIDE the step container as a sibling element, always visible regardless of step fold state. When the parts contain only `text` blocks, the assistant turn SHALL render without a step wrapper (preserving the existing inline rendering). When `parts` is empty, the existing `(empty turn)` placeholder SHALL render.
...
#### Scenario: 含 thinking + tool + final response 时: fold 包 inference,最终 text 在 fold 外
- **GIVEN** an assistant message with `parts: [thinking, toolCall, toolResult, text]`
- **WHEN** the MessageParts component renders
- **THEN** a step header button appears in the DOM; the fold body contains the inference parts (ThinkingItem, ToolGroup) in original order; the LAST text part (the agent's final conclusion) renders OUTSIDE the step container as a sibling element, after the fold
...
#### Scenario: 多个 text block 中间夹 tool: 中间 text 在 fold 内,最终 text 在 fold 外
- **GIVEN** an assistant message with `parts: [thinking, text("interim"), toolCall, toolResult, text("final")]`
- **WHEN** the MessageParts component renders
- **THEN** the fold body contains thinking → `text("interim")` → ToolGroup in original chronological order; `text("final")` (the LAST text part, the turn's final conclusion) renders OUTSIDE the fold as a sibling TextItem element after the fold. Intermediate text emitted between inference parts is treated as inference and stays inside the step body.

### Requirement: Final Text Outside Fold Wrapper

The LAST `text` part in an assistant message (the agent's final conclusion) SHALL render OUTSIDE the step wrapper as a sibling element, regardless of fold state. This text SHALL be visible both when the fold is expanded (during streaming) and when the fold is collapsed (after streaming ends). This is the primary user-visible guarantee: the agent's final reply is never hidden inside a collapsed step. Intermediate `text` parts emitted between inference parts (thinking / tool calls) are treated as inference and render INSIDE the fold body.

#### Scenario: 推理过程展开时,最终 text 在 fold 外可见
- **GIVEN** a turn with `parts=[thinking, toolCall, toolResult, text("final")]`, `isStreaming=true`
- **WHEN** MessageParts renders
- **THEN** the fold is open; the text "final" (the LAST text part) is visible in a TextItem that lives OUTSIDE the fold element (sibling, not descendant)

#### Scenario: 推理过程自动折叠后,最终 text 仍可见 (核心 spec)
...
The step body (the fold contents — inference parts and intermediate text) SHALL be visible (expanded) by default when `isStreaming=true` and hidden (collapsed) by default when `isStreaming=false`. The user SHALL be able to click the step header to toggle the body visibility. Once the user has clicked, the user's choice SHALL persist for the lifetime of that step instance; subsequent changes to `isStreaming` SHALL NOT override the user's choice. **The final text part (the LAST text part in the turn) is not part of the step body and is unaffected by fold state — it remains visible whether the fold is open or closed. Intermediate text parts (text emitted between inference parts) ARE part of the step body and are affected by fold state.**

#### Scenario: Streaming 时 body 默认展开
- **GIVEN** a step with `isStreaming=true`
- **WHEN** the step renders
- **THEN** the step body is visible (inference parts + intermediate text rendered in chronological order). The final text part (the LAST text part in the turn) is also visible (it lives outside the fold)

#### Scenario: Done 后 body 自动折叠,最终 text 仍可见
- **GIVEN** a step where `isStreaming` was `true` and the body was visible
- **WHEN** `isStreaming` becomes `false`
- **THEN** the body auto-collapses (inference parts + intermediate text removed from the DOM); the header remains visible. **The final text part (rendered outside the fold) remains visible regardless of fold state**

#### Scenario: 用户点击 header 切换
- **GIVEN** a step with `isStreaming=false`, body collapsed
- **WHEN** the user clicks the step header button
- **THEN** the body becomes visible; clicking again hides it. The final text part is unaffected by the click and remains visible

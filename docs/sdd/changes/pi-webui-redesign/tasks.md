...
- [ ] A.4 **`readMessages` 返回 usage 字段 + 测试**
...
- [ ] A.5 **`spawnPiNewSession` + UUID 降级 + 测试**
...
- [ ] A.6 **`POST /api/sessions` 用 `spawnPiNewSession`**
...
- [ ] A.8 **`session-pool.prompt` 透传 `content[]` 给 pi**
...
## Phase B: Web 工具 + 原子组件 (大部分可并行)

- [ ] B.0 **从归档导入 `MessageParts.tsx` + `MessageParts.test.tsx`**
...
- [ ] B.1 **`formatToken` + `formatRelativeTime` 工具 + 单测**
...
- [ ] B.2 **`validateImageFile` + `fileToBase64` + 单测**
...
- [ ] B.8 **`ModelSelector` 组件 + 测试**
  ...
  - **依赖**: B.1 (formatToken 不需要, truncate 自己写)

- [ ] B.9 **`MessageHeader` 组件 + 测试**
...
- [ ] C.1 **`api.ts` 扩展 (`Message` 类型重写 + `ModelsResponse` + `InputImage` + 4 个 API helper)**
  ...
  - **依赖**: 无 (但 B.9, B.10, B.11, C.4 都依赖此任务)

- [ ] C.2 **`AppShell` 2-col 布局**
...
- [ ] C.4 **`ChatPage` 重写 - 顶栏 + 输入 + 消息**
  ...
  - **依赖**: C.1, C.2, B.0, B.7, B.8, B.11, B.13, B.14

- [ ] C.5 **`InputArea` 整合 (preview + input + text)**
...
- [ ] C.7 **`GET /api/settings` 端点 (用于 main.tsx 加载)**

# 020: RPC Mode with Typed Protocol
...
The original RPC mode used a loosely typed JSON protocol with ad-hoc command handling. Commands were strings, responses were untyped objects, and there wasn't any client library for programmatic access. External tools (IDEs, CI scripts, other agents) that wanted to drive the coding agent had to parse raw JSON and guess the response shape. The refactored `AgentSession` API (ADR-019) provided a clean surface to expose.

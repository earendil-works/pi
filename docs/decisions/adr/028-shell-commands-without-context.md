...
The bash mode (`!` prefix, ADR-015) executed shell commands and included their output in the LLM context. This was useful for commands the LLM needed to know about. But many quick commands — `ls`, `git status`, `curl` a URL — produced output the user wanted to see without bloating the context window or distracting the model. There was no way to run a command silently.

...
The `/branch` command created a new conversation path within a session tree. The name "branch" collided with git terminology: users in a git repository would reflexively type `git branch` and get confused when the agent interpreted it as a session command instead of a git operation. The session tree (ADR-023) already used tree and branching metaphors, but "branch" was overloaded.

...
Entry: `src/cli.ts`. Modes: Interactive (TUI), Print (JSON), RPC (inter-process), SDK (embedded). Extensions, skills, prompt templates, and themes load from `~/.pi/` or the config path.

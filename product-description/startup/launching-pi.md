...
5. **The changelog box**, only after an upgrade: a bordered block with `What's New` in the accent colour and the changelog entries newer than the last version that ran, as markdown. With `collapseChangelog` it is one line: `Updated to v0.84.1. Use /changelog to view full changelog.` On a fresh install nothing is shown and the version is recorded; on resume (`-c`, `-r`, `--session` of a session with messages) it is never shown. One anonymous install ping is sent with it unless `enableInstallTelemetry` is off or `--offline`.

...
**Branching and history.** The entries are part of the branch they were made on. Moving the active position with `/tree` restores the model recorded on that branch; the selector then shows that model as current. The `/model` line is in the prompt history like any submitted line.

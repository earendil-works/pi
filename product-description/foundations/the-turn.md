...
- **No credential for any provider.** The turn fails before anything is sent with `Error: No API key found for the selected model.` followed by the login instructions. The prompt is not recorded and not drawn; the text is gone from the editor and is in the prompt history.
- **The selected model's provider has no credential** (logged out, or a key removed from the environment). Same shape, naming the provider: `Error: No API key found for anthropic.`, or for OAuth providers `Authentication failed for "anthropic". Credentials may have expired or network is unavailable. Run '/login anthropic' to re-authenticate.` Not recorded.
...
| Ctrl+C once / twice; Ctrl+D | One Ctrl+C clears the editor; it does not touch the turn. Two within 500 ms quit pi; see [quitting](../sessions/quitting.md). Ctrl+D quits only with an empty editor. | Same. Quitting mid-turn aborts the turn but does not wait for the abort to settle before exiting, so the partial assistant message and the running tools' results may or may not reach the session file; everything completed before the quit is on disk. |
